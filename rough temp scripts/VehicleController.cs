...
public class VehicleController : MonoBehaviour
{
    ...
    private const float SMC_LP = 2.0f;  // look ahead point [m]
    ...
    private void FixedUpdate()
    {
        ...
        Debug.Log("linear: " + ref_vel[0].ToString("F3") + "[m/s],   angular: " + ref_vel[1].ToString("F3") + "[rad/s] control input");
    ...
    float[] SMC(float[] refPoint)
    {
        ...
        float[] result_vel = new float[2] { 0.3f, 0.0f }; // [m/s], [rad/s]
    ...
    float[] TargetPointCalc(float Ld = 1)
    {
        ...
        float lp_x = model.position.x + SMC_LP * Mathf.Cos(model.rotation.y);
        float lp_z = model.position.z + SMC_LP * Mathf.Sin(model.rotation.y);

        if (model.rotation.y < Mathf.Deg2Rad*10.0f)
        {
